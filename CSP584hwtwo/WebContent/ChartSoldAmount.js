...
//Bind click event to make an ajax call to post request of ChartSoldAmount. This will return
// a json object with top 3 review for each city;
$("#btnGetChartSoldAmount").click(function () {
    $("#btnGetChartSoldAmount").hide();
    $.ajax({
        url: "SalesAmount",
        type: "POST",
        data: "{}",
...
function createDataTable(jsonData) {
    ...
    for (var i = 0; i < soldAmountArr.length; i++) {
        var dataArr = new Array(headingArray.length);
        dataArr[0] = soldAmountArr[i];
        for (var j = 0; j < productNameArr.length; j++) {
            for (k = 0; k < parsedData.length; k++) {
                if (parsedData[k]["soldAmount"] === soldAmountArr[i] && parsedData[k]["productName"] === productNameArr[j]) {
